var ApplicationOptions={colors:{states:{NORMAL:"blue",WARNING:"orange",OFFLINE:"grey",ALERTED:"red",UNKNOWN:"black"},application:{header:"grey"}},constants:{SPEED_HISTORY_COUNT:20,NOTIFY_INFO_TIMEOUT:1E3,NOTIFY_SUCCESS_TIMEOUT:1E3,NOTIFY_WARNING_TIMEOUT:3E3,NOTIFY_DANGER_TIMEOUT:5E3},messages:{app:{}},leaflet:{iconUrls:{normalIcon:"assets/img/markers/arrow_normal.png",alertedIcon:"assets/img/markers/arrow_alerted.png",offlineIcon:"assets/img/markers/arrow_offline.png",warningIcon:"assets/img/markers/arrow_warning.png",
defaultIcon:"assets/img/markers/default_icons/marker-icon.png",resizeIcon:"assets/img/markers/resize.png"}}};var debugObject,showPathFlag=!1,currentSpatialObjects={},selectedSpatialObject,webSocketURL=ApplicationOptions.constants.WEB_SOCKET_URL,websocket,waitTime=1E3;
...
var webSocketOnOpen=function(){$.UIkit.notify({message:"You Are Connectedto Map Server!!",status:"success",timeout:ApplicationOptions.constants.NOTIFY_SUCCESS_TIMEOUT,pos:"top-center"})},webSocketOnError=function(a){$.UIkit.notify({message:"Something went wrong when trying to connect to <b>"+webSocketURL+"<b/>",status:"danger",timeout:ApplicationOptions.constants.NOTIFY_DANGER_TIMEOUT,pos:"top-center"})},webSocketOnClose=function(a){$.UIkit.notify({message:"Connection lost with server!!",status:"danger",
timeout:ApplicationOptions.constants.NOTIFY_DANGER_TIMEOUT,pos:"top-center"});waitForSocketConnection(websocket)},webSocketOnMessage=function(a){a=$.parseJSON(a.data);if(a.id in currentSpatialObjects)currentSpatialObjects[a.id].update(a);else{var b=new SpatialObject(a);b.update(a);currentSpatialObjects[b.id]=b;currentSpatialObjects[b.id].addTo(map)}};
...
function notifyAlert(a){$.UIkit.notify({message:"Alert: "+a,status:"warning",timeout:ApplicationOptions.constants.NOTIFY_WARNING_TIMEOUT,pos:"bottom-left"})}function Alert(a,b,c){this.type=a;this.message=b;this.level=c?c:"info";this.notify=function(){$.UIkit.notify({message:this.level+": "+this.type+" "+this.message,status:"info",timeout:ApplicationOptions.constants.NOTIFY_INFO_TIMEOUT,pos:"bottom-left"})}}
...
map.addLayer(f);$.post("controllers/tile_servers.jag",{url:a,name:b,attribution:d,maxzoom:c,subdomains:subdomains},function(a){$.UIkit.notify({message:'<span style="color: dodgerblue">'+a+"</span>",status:"success",timeout:ApplicationOptions.constants.NOTIFY_SUCCESS_TIMEOUT,pos:"top-center"});closeAll()})}
...
function getTileServers(){$.getJSON("controllers/tile_servers.jag?serverId=all",function(a){$.each(a,function(a,c){$.UIkit.notify({message:'Loading... <span style="color: #ccfcff">'+c.name+'</span> URL: <span style="color: #00ff00">'+c.url+"</span>",status:"info",timeout:ApplicationOptions.constants.NOTIFY_INFO_TIMEOUT,pos:"bottom-left"});var d=L.tileLayer(c.url,{maxZoom:c.maxzoom,subdomains:c.subdomains.split(","),attribution:c.attribution});layerControl.addBaseLayer(d,c.name)})})}
...
layers:layers,wmsVersion:wmsVersion,serviceEndPoint:serviceEndPoint,outputFormat:outputFormat},function(a){$.UIkit.notify({message:'<span style="color: dodgerblue">'+a+"</span>",status:"success",timeout:ApplicationOptions.constants.NOTIFY_SUCCESS_TIMEOUT,pos:"top-center"});closeAll()})}
